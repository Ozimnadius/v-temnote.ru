...
```
Повторно используемые стили блока добавляй партиалом в `css/blocks/` и регистрируй его в `css/blocks/_index.scss`; та же схема для `css/ui/_index.scss`. Файлы `_#sample.scss` — это шаблоны-заготовки для копирования.

Стили, используемые только на одной странице, размещай в `css/pages/<page>.scss`, компилируй в соседний `<page>.css` и подключай через `Bitrix\Main\Page\Asset` непосредственно в PHP-файле этой страницы. Page-only стили не добавляй в общий `styles.scss` или `css/blocks/_index.scss`.
