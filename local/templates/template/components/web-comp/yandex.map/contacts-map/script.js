(function () {
  ...
  const ERROR_CLASS = "contacts-map__frame--error";
      ...
      ymapsReadyPromise = new Promise((resolve, reject) => {
        ...
        function checkReady() {
          ...
          if (Date.now() > deadline) {
            reject(new Error(`ymaps3 не загрузился за ${READY_TIMEOUT_MS} мс`));
            return;
          }
  ...
  // initMap никогда не реджектится (всё после guard'а накрыто try/catch),
  // поэтому fire-and-forget вызов из forEach безопасен.
  async function initMap(mapNode) {
    const center = [
      Number(mapNode.dataset.centerLng),
      Number(mapNode.dataset.centerLat)
    ];
    const zoom = Number(mapNode.dataset.zoom) || DEFAULT_ZOOM;
    const placemarkLabel = mapNode.dataset.placemarkLabel || "";

    if (center.some((value) => !Number.isFinite(value))) {
      mapNode.classList.add(ERROR_CLASS);
      return;
    }
    ...
    try {
      ...
      mapNode.classList.remove(ERROR_CLASS);
  ...
  function init() {
    document.querySelectorAll("[data-yandex-map]").forEach((mapNode) => initMap(mapNode));
  }
